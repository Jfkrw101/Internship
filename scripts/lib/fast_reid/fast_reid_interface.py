...
from lib.fast_reid.fastreid.modeling.meta_arch import build_model
...
class FastReIDInterface:
    ...
    def inference(self, image, detections):
        ...
        for d in range(np.size(detections, 0)):
            xywh = detections[d, :4].astype(np.int_)
            tlbr = np.array([
                max(0, xywh[0]),                # top-left x
                max(0, xywh[1]),                # top-left y
                min(W - 1, xywh[0] + xywh[2]),  # bottom-right x
                min(H - 1, xywh[1] + xywh[3])   # bottom-right y
            ])
