import cv2
from lib.pose_estimate import PoseEstimate
import os
import time
import GPUtil
import threading
...
def pose_estimation(model,vdo_path, vdo_idx, log_path, output_vdo_path):
    cap = cv2.VideoCapture(vdo_path)
    pose_estimate = PoseEstimate(model)
    frame_width = int(cap.get(3))
    frame_height = int(cap.get(4))
    size = (frame_width, frame_height)
    fps = cap.get(cv2.CAP_PROP_FPS)
    vdo_output_dir = output_vdo_path 
    output_filename = f"pose_estimate_{vdo_idx + 1}.mp4"
    os.makedirs(vdo_output_dir, exist_ok=True)
    output_path = os.path.join(vdo_output_dir, output_filename)
    result_vid = cv2.VideoWriter(output_path,cv2.VideoWriter_fourcc(*"H264"),fps,size)
    while cap.isOpened:
        success, frame = cap.read()
        if success:
            pose_estimate.frame = frame
            data, annotated_frame, results = pose_estimate.estimator(frame)
            result_vid.write(annotated_frame)
...
def multithread_pose_estimation(vdo_dir, model, log_path, output_vdo_path):
    ...
    for i, vdo_file in enumerate(origin_dir):
        ...
        thread = threading.Thread(target=pose_estimation, args=(model,vdo_path, i, log_path, output_vdo_path), daemon=True)
...
if __name__ == "__main__":
    ...
    multithread_pose_estimation(vdo_path, model_path, log_path, saved_dir)
