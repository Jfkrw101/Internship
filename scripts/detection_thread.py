...
def detection(model,vdo_path, vdo_idx, log_path, output_vdo_path):
    ...
    output_filename = f"detection_{vdo_idx + 1}.mp4"
    ...
    while cap.isOpened:
        ...
        if success:
            ...
            data, annotated_frame, results = obj_detection.detector(frame)
...
def multithread_detection(vdo_dir, model, log_path, output_vdo_path):
    ...
    for i, vdo_file in enumerate(origin_dir):
        ...
        thread = threading.Thread(target=detection, args=(model,vdo_path, i, log_path, output_vdo_path), daemon=True)
...
if __name__ == "__main__":
    ...
    multithread_detection(vdo_path, model_path, log_path, saved_dir)
