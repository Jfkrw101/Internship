...
def pose_tracking(model,tracker,vdo_path, vdo_idx, log_path, output_vdo_path):
    ...
    output_filename = f"pose_tracking_{vdo_idx + 1}.mp4"
    ...
    while cap.isOpened:
        ...
        if success:
            ...
            data, _, results = pose_estimate.estimator(frame)
...
def multithread_tracking(vdo_dir, model, tracker,log_path, output_vdo_path):
    ...
    for i, vdo_file in enumerate(origin_dir):
        ...
        thread = threading.Thread(target=pose_tracking, args=(model,tracker,vdo_path, i, log_path, output_vdo_path), daemon=True)
